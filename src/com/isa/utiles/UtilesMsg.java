/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.isa.utiles;

/**
 *
 * @author JMiraballes
 */
public class UtilesMsg {
    
    
    public static String DOC_FIRMADO_OK = "Se ha firmado correctamente el documento.";
    public static String PENDIENTE = "Usted no tiene ningún certificado instalado.";
    public static String ERROR_FIRMAR_DOC = "Ha ocurrido un error y no se ha podido firmar el documento.";
    public static String ERROR_PIN_INCORRECTO = "El pin no es correcto.";
    public static String ERROR_TOKEN_BLOQUEADO = "El token se ha bloquedo.";
    public static String ERROR_TOKEN_SIN_IDENTIFICAR = "No se puede identificar el token.";
    public static String PROCESANDO_FIRMA = "Procesando firma...";
    public static String FIRMA_VERIFICADA_OK = "Firma verificada correctamente.";
    public static String VALIDANDO_DOCUMENTO = "Validando documento...";
    public static String ERROR_FIRMA_NO_VALIDA = "La firma no es válida.";
    public static String ERROR_UNA_FIRMA_NO_VALIDA = "Una o más firmas en el documento no son válidas.";
    public static String ERROR_CARGANDO_CERTIFICADOS = "Error cargando certificados.";
    public static String ERROR_VERIFICANDO_FIRMA = "Ha ocurrido un error y no se ha podido verificar la firma.";
    public static String ERROR_AUTH = "Ha ocurrido un error en la autenticación"; 
    
    public static String ERROR_ACCEDIENDO_ARCHIVO = "Ha ocurrido un error accediendo a los archivos de configuración";
    public static String ERROR_ACCEDIENDO_TOKEN = "No se ha podido obtener la información del token.";
    public static String ERROR_ACCEDIENDO_PROVEEDOR = "No se ha podido acceder al token.";
    public static String ERROR_CERT_NO_SELECCIONADO = "Debe seleccionar un certificado.";
    public static String ERROR_WS_EXCEPTION = "Ha ocurrido un error en los servicios web.";
    public static String ERROR_WS_EXCEPTION_VALIDACION = "Ha ocurrido un error en los servicios web de validación";
    public static String ERROR_FIRMANDO_DOCUMENTO = "Ha ocurrido un error firmando el documento.";
    public static String ERROR_GUARDANDO_DOCUMENTO = "Ha ocurrido un error al guardar el documento. Código -1.";
    public static String ERROR_CODIGO_WS = "Código devuelto por web service es incorrecto.";
    public static String ERROR_RESPUESTA_WS = "La respuesta del servicio web no es correcta.";
    public static String DOCUMENTO_GUARDADO_OK = "Documento guardado correctamente.";
    public static String DOCUMENTO_FIRMADO_GUARDADO_OK = "Documento guardado y firmado correctamente.";
    public static String ERROR_CONEXION_WEB_SERVICES = "Ocurrió un error de conexión con los servicios web.";
}
